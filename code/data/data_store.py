...
def store_data(algorithm) -> None:
    """
    Stores the time and stability data generated by the given algorithm into a CSV file.
    The data is gathered using the `gather_data` function and then written to a CSV file named 
    after the algorithm in the "code/data/many_runs/" directory. If the file already exists, 
    the data is appended to the file; otherwise, a new file is created with headers.

    Args:
    ------
        algorithm (function): The algorithm function whose data is to be stored. The function 
                              should have a `__name__` attribute to name the CSV file.
    """
